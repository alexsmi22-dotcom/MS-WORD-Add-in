...
- **Table figure** — draw the **table itself** as a clean figure, for characteristics/reference tables where the table *is* the exhibit. Preserves **section grouping** (a group-header row becomes a band; a blank "section" column merges down its rows), bolds the header, word-wraps cells, and scales wide tables to fit. Handles the dense clinical-style tables (`Section | Characteristic | n (%)`) common in specs.
- **Insert as a figure** at the cursor — the graphic goes straight into your document as an image. Tables with no numeric data switch to a table figure automatically.
