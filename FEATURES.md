...
## 🏗️ Engineering — statics, mechanics of materials, circuits & thermofluids
- Eleven calculators over your own numbers, offline. **Exact where exactness is real, floating point where it is not, and it always says which.**
...
- Honest limits, disclosed in-pane: beams are prismatic, linearly elastic, small-deflection Euler-Bernoulli (no shear deformation); circuits are linear and lumped, with no transient analysis; trusses are pin-jointed and statically determinate only; the pipe and heat tools are strict SI. This computes the mechanics — it does not tell you whether a member passes a design code.
